...
const statusColors: Record<string, string> = {
  new: 'bg-primary-container/10 text-primary-container',
  contacted: 'bg-surface-container-high text-on-surface-variant',
  booked: 'bg-primary-container/5 text-primary-container',
  archived: 'bg-surface-container text-on-surface-variant/60',
};
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <h2 className="text-[22px] font-bold text-on-surface">Leads</h2>
          <p className="text-sm text-on-surface-variant mt-1">Students who have contacted you about your services.</p>
        </div>
        <div className="bg-white rounded-lg border border-outline-variant shadow-sm p-1 flex gap-1">
          {tabs.map((tab) => (
            <button
              key={tab}
              onClick={() => setActiveTab(tab)}
              className={`px-4 py-2 rounded-md text-xs font-bold transition-all ${
                activeTab === tab ? 'bg-primary-container text-white shadow-sm' : 'text-on-surface-variant hover:text-primary-container'
              }`}
            >
              {tab}
            </button>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center bg-white rounded-2xl border border-outline-variant shadow-sm">
          ...
          <h3 className="text-lg font-bold text-on-surface mb-1">No leads yet</h3>
        ...
        <div className="space-y-3">
          {filtered.map((lead) => (
            <div key={lead.id} className="bg-white rounded-2xl border border-outline-variant shadow-sm p-5 hover:shadow-md transition-shadow duration-200">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-3 mb-1">
                    <h3 className="text-sm font-bold text-on-surface">{lead.name}</h3>
                  ...
                  <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-on-surface-variant/60">
                    {lead.suburb && (
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  {lead.status === 'new' && (
                    <button onClick={() => updateLeadStatus(lead.id, 'contacted')} className="bg-primary-container text-white px-4 py-2 rounded-lg text-xs font-bold hover:bg-primary-container/90 transition-all whitespace-nowrap">Mark Contacted</button>
                  )}
                </div>
              </div>
            </div>
