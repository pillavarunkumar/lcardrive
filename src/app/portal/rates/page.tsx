      ...
      <div className="grid grid-cols-12 gap-gutter">
        <div className="col-span-12 lg:col-span-7 space-y-gutter">
          <section className="bg-white p-8 rounded-2xl border border-outline-variant shadow-sm">
            <div className="flex items-start justify-between mb-6">
              <div>
                <h2 className="text-base font-semibold text-on-surface mb-1">Standard Rate</h2>
                <p className="text-sm text-on-surface-variant">Set your base hourly rate for individual lessons.</p>
              </div>
            ...
            <div className="relative max-w-xs">
              ...
              <input
                className="w-full pl-8 pr-16 py-4 bg-surface-container-low border border-outline-variant rounded-lg text-[32px] font-bold text-on-surface focus:ring-2 focus:ring-primary-container focus:border-transparent transition-all outline-none"
                type="number"
                value={hourlyRate}
                onChange={(e) => setHourlyRate(parseInt(e.target.value) || 0)}
              />
              <span className="absolute right-4 top-1/2 -translate-y-1/2 text-on-surface-variant text-sm">/ hr</span>
            </div>
            <p className="mt-4 text-sm text-on-surface-variant/60 italic">Average instructor rate in your area: $65 — $75/hr</p>
          </section>

          <section className="bg-white p-8 rounded-2xl border border-outline-variant shadow-sm">
            <div className="flex items-start justify-between mb-8">
              <div>
                <h2 className="text-base font-semibold text-on-surface mb-1">Lesson Packages</h2>
                <p className="text-sm text-on-surface-variant">Offer discounts for multi-lesson bookings to encourage commitment.</p>
              </div>
              <span className="material-symbols-outlined text-on-surface-variant text-3xl">redeem</span>
            </div>
            <div className="space-y-6">
              {packages.map((pkg, i) => (
                <div key={i} className="flex flex-col md:flex-row md:items-center justify-between p-6 bg-surface-container-low rounded-lg border border-outline-variant">
                  <div>
                    <h3 className="text-sm font-bold text-on-surface">{pkg.label}</h3>
                    <p className="text-sm text-on-surface-variant">Ideal for beginners starting their journey</p>
                  </div>
                  <div className="flex items-center gap-4 mt-4 md:mt-0">
                    <div className="text-right">
                      <p className="text-[10px] text-on-surface-variant uppercase tracking-wide font-semibold">Package Price</p>
                      <div className="flex items-center gap-2">
                        <span className="text-on-surface font-bold">$</span>
                        <input
                          className="w-24 px-2 py-1 bg-white border border-outline-variant rounded focus:ring-1 focus:ring-primary-container outline-none text-right font-bold"
                          type="number"
                          value={pkg.price}
          ...
          <div className="flex items-center justify-end gap-4 pt-4">
            <button className="px-8 py-3 text-on-surface-variant font-bold hover:text-primary-container transition-colors rounded-lg">Discard Changes</button>
            <button
              onClick={handleSave}
              disabled={saving}
              className="px-10 py-3 bg-primary-container text-white rounded-xl font-bold hover:bg-primary-container/90 transition-all flex items-center gap-2 disabled:opacity-50"
            >
        ...
        <div className="col-span-12 lg:col-span-5">
          <div className="sticky top-28">
            <div className="bg-primary-container text-white rounded-2xl overflow-hidden shadow-2xl relative">
            ...
            <div className="mt-gutter p-6 bg-white border border-outline-variant rounded-2xl shadow-sm">
              <h4 className="font-bold text-on-surface mb-2 flex items-center gap-2">
                <span className="material-symbols-outlined text-sm">info</span>
                Revenue Projections
              </h4>
              <p className="text-sm text-on-surface-variant mb-4">Based on your new rates and current booking volume (avg. 22 hrs/week):</p>
              <div className="grid grid-cols-2 gap-4">
                <div className="p-3 bg-surface-container-low rounded-lg">
                  <p className="text-[10px] text-on-surface-variant uppercase tracking-wide font-semibold">Est. Monthly</p>
                  <p className="text-xl font-bold text-on-surface">${(hourlyRate * 22 * 4).toLocaleString()}</p>
                </div>
                <div className="p-3 bg-surface-container-low rounded-lg">
                  <p className="text-[10px] text-on-surface-variant uppercase tracking-wide font-semibold">Platform Fee (5%)</p>
