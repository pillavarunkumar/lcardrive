...
export default function VerificationStatus({ formState, onVerify, isVerified }: Props) {
  ...
  return (
    <div className={`rounded-xl p-6 border ${isVerified ? 'bg-primary-container/30 border-primary/20' : 'bg-surface-container-lowest border-outline-variant/20'} shadow-[0px_4px_20px_rgba(15,23,42,0.04)]`}>
      <div className="flex items-center gap-3 mb-4">
        <div className={`rounded-full p-1.5 flex items-center justify-center ${isVerified ? 'bg-primary text-white' : 'bg-surface-variant text-on-surface-variant'}`}>
      ...
      <div className="space-y-2 mb-4">
        <div className="flex justify-between items-center font-label-sm text-label-sm">
          ...
          <span className={isVerified ? 'text-primary font-bold' : 'text-on-surface-variant font-bold'}>{passedCount}/{totalCount}</span>
        </div>
        <div className="w-full bg-surface-container-high rounded-full h-2 overflow-hidden">
          <div
            className={`h-2 rounded-full transition-all duration-500 ${isVerified ? 'bg-primary' : 'bg-primary/60'}`}
      ...
      <div className="space-y-1.5">
        {result.checks.map((check) => (
          <div key={check.label} className="flex items-center gap-2 font-label-sm text-label-sm">
            <span
              className={`material-symbols-outlined text-[16px] ${check.passed ? 'text-primary' : 'text-on-surface-variant/50'}`}
      ...
      {!isVerified && result.fullyVerified && onVerify && (
        <button
          onClick={onVerify}
          className="mt-4 w-full bg-primary hover:bg-primary/90 text-white rounded-lg px-4 py-2 font-label-md text-label-md transition-colors shadow-sm flex items-center justify-center gap-2"
        >
